...
Mesh-сеть из узлов **ESP32 + LoRa**, где каждый узел — это автономный участник сети:

### Типы Узлов

| Тип | Node ID | Описание |
|-----|---------|----------|
| 🖥️ **Server Node** | 100 | Хранение почты, файлов, маршрутизация |
| 💻 **Client Node** | 101-254 | Пользовательские терминалы |

### Ключевые Возможности

- 🔄 **Авто-маршрутизация** — узлы находят друг друга автоматически
- 📦 **Store & Forward** — ретрансляция пакетов через промежуточные узлы
- 🔐 **E2E Шифрование** — AES-128-GCM + Ed25519 подписи
- 📡 **LoRa Mesh** — до 10 км прямой видимости
